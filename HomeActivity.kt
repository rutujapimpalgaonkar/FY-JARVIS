...
import com.plgpl.jarvis.domain.Event
import com.plgpl.jarvis.helpers.ApkInfoExtractor
import com.plgpl.jarvis.helpers.ContactHelper
import com.plgpl.jarvis.helpers.LocationHelper
import com.plgpl.jarvis.helpers.ViewsHelper
import com.plgpl.jarvis.services.CalendarService
import com.plgpl.jarvis.services.PreferencesService
import com.plgpl.jarvis.services.WeatherService
import java.time.*
...
class HomeActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    fun procesa(text: String) {
        ...
        if (lowerText == "what is my location" || lowerText == "what's my location"
            ...
            location?.let {
                WeatherService(this).getCurrentWeatherByLocation(
                    null, null,
            ...
            PreferencesService(this).getPreference("city")?.let {
                WeatherService(this)
            ...
            if (city == "my default location") {
                ...
                WeatherService(this).getCurrentWeatherByLocation(
                    null, null, city, findViewById(R.id.weather),
            ...
            PreferencesService(this).getPreference("city")?.let {
                WeatherService(this)
                    .getForecastWeatherByLocation(
                        ...
                        null,
                        it,
                        findViewById(R.id.weatherForecast),
                        false
                    )
            }
            ...
            if (city2!!.isNotBlank()) {
                WeatherService(this)
                    .getForecastWeatherByLocation(
                        ...
                        null,
                        city2,
                        findViewById(R.id.weatherForecast2),
                        false
                    )
            }

            viewMessage("Query successful: weather forecast", true)
        } else if ((lowerText.startsWith("what is the weather forecast in") || lowerText.startsWith(
                "what's the weather forecast in"
            ))
        ) {
            ...
            if (city == "my default location") {
                ...
                location?.let {
                    WeatherService(this).getForecastWeatherByLocation(
                        null, null,
            ...
            val app = lowerText.split("open")[1].trim()
            val appExtractor = ApkInfoExtractor(applicationContext);
            val allNames = appExtractor.getAllName();
            if (allNames.keys.contains(app)) {
                val packageName = allNames.get(app)
                val launchIntent = packageManager.getLaunchIntentForPackage(packageName!!)
                startActivity(launchIntent)
            } else {
                viewMessage("Sorry, Can't find app with that name", false)
                reprodueixSo("Sorry, Can't find app with that name")
            }
        } else if (lowerText.startsWith("call")) {
            val contact = lowerText.split("call")[1].trim()
            val allPhoneNumbers = ContactHelper.getPhoneNumbers(applicationContext)
            if (allPhoneNumbers.keys.contains(contact)) {
                val number = allPhoneNumbers.get(contact)
    ...
    fun mostraInfoInicial() {
        ...
        var city2 = PreferencesService(this).getPreference("city2")
    ...
    private fun actualitzaVistaLocation(latitude: Double, longitude: Double) {
        ...
        val location =
            "${city.ciutat}, ${city.codiPais}${System.getProperty("line.separator")}($latitude,$longitude)"
        text.text = location
    ...
    private fun mostraCalendari(avui: Boolean, speak: Boolean) {
        ...
        if (esdeveniment != null) {
            ...
            val initTime =
                Instant.ofEpochMilli(esdeveniment.initTime).atZone(ZoneId.systemDefault())
                    .toLocalDateTime()
            val endTime = Instant.ofEpochMilli(esdeveniment.endTime).atZone(ZoneId.systemDefault())
                .toLocalDateTime()
            ...
            if (esdeveniment.allDay) {
                ...
                missatge =
                    initTime.format(dateFormatter) + " - " + endTime.format(dateFormatter) + ": " + esdeveniment.title
    ...
    private fun mostraDialogEvent() {
        ...
        builder.setPositiveButton("Submit") { dialog, which ->
            ...
            val initTime =
                LocalDateTime.parse(dataInici.text, DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm"))
            ...
            if (allDay.isChecked) {
                event.initTime = LocalDateTime.parse(
                    dataInici.text,
                    DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")
                )
                    .plusDays(1).toLocalDate().atStartOfDay(ZoneId.systemDefault()).toInstant()
                    .toEpochMilli()

                event.endTime = LocalDateTime.parse(
                    dataInici.text,
                    DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")
                )
                    .plusDays(2).toLocalDate().atStartOfDay(ZoneId.systemDefault()).toInstant()
                    .toEpochMilli()
